...
// إنشاء مستخدم - الصلاحيات تتحدد تلقائياً من الدور
router.post('/create-user', async (req, res) => {
    try {
        ...
        // الصلاحيات تأتي تلقائياً من الدور
        ...
        res.status(201).json({
            success: true,
            message: 'تم إنشاء المستخدم - الصلاحيات تحددت تلقائياً',
            data: {
                id: newUser._id,
                username,
                email,
...
router.post('/login', async (req, res) => {
    try {
        ...
        const token = jwt.sign(
            { userId: user._id, username, roleId: user.role._id, dept: user.department._id },
...
router.put('/update-user-permissions/:userId', async (req, res) => {
    try {
        ...
        if (!permissionIds || permissionIds.length === 0) {
            return res.status(400).json({ success: false, message: '��لصلاحيات مطلوبة' });
        }
        ...
        const user = await User.findByIdAndUpdate(
            ...
            { new: true }
        ).populate('customPermissions').populate('role').populate('department');
...
// تعديل دور المستخدم (الصلاحيات تحدث تلقائياً)
router.put('/update-user-role/:userId', async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            message: 'تم تحديث الدور والصلاحيات تلقائياً',
            data: {
                id: user._id,
                username: user.username,
                role: user.role.name,
